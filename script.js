...
class AddressBook {
  ...
  init(){
    ...
    const expDate = getCookie('storageExpiration');
    if(expDate < today || expDate === today) {
      localStorage.removeItem('contacts');
      deleteCookie('storageExpiration');
    }
    this.returnAll();
  }
  removeContact(name) {
  ...
  setStorage(key, value){
    const date = Date.now();
    const expDate = new Date(date + 1000 * 60 * 60 * 24 * 10);
    setCookie('storageExpiration', expDate.toUTCString());
